...
/* 仙加味顧客DM權威層｜2026-08-12 corrected-v1
 * 產品圖與DM分開：產品頁／產品卡使用六張正式產品圖；DM欄位只使用各產品詳細DM。
 * 30cc／龜鹿湯塊／龜鹿膠的舊字已於本批DM修正後才切換。
...
(function(){
  ...
  const VERSION='20260812-dm-corrected-v1';
  const DM=Object.freeze({
    'guilu-gao':'images/dm-approved-v20260810/guilu-gao-100g.webp',
    ...
    try{
      ...
      if(Array.isArray(data.products)){
        data.products=data.products.map(product=>{
          ...
          return dm?{...product,dmImage:`${dm}?v=${VERSION}`,dmImagePolicy:'separate-corrected-detailed-dm-contain-no-crop'}:product;
        });
      }
      data.runtime={...(data.runtime||{}),dmSource:'separate-corrected-detailed-dm',dmVersion:VERSION,trialImage:`${TRIAL}?v=20260812-six-product-final-v3-clean`,dmRoleRule:'product image / detailed DM / trial image are separate media roles'};
