...
// Current runtime uses compatibility media layers only to enrich current posts.
// Historical layers may remain for traceability but may not become product/copy authority or force old fixed counts.
...
assert.equal(auth.knowledgeProducts,7,'目前貼文文字產品不是七項');
assert.equal(auth.approvedMediaProducts,6,'目前核准產品媒體不是六項');
assert.equal(auth.drink30Usage,CURRENT_30,'30cc目前用法不是每日 1–2 罐');
assert.equal(auth.drink180Usage,'每日一包','180cc目前用法不是每日一包');
assert.ok(!Object.hasOwn(auth,'sixProductsSixSpecs'),'舊六產品權威欄位仍在runtime');
...
assert.match(guard.validateFormalCopy({id:'POST-PRODUCT-OVERVIEW',copy:'目前有六個正式產品'}),/舊六產品模型/,'舊六產品總覽仍必須被擋');
...
assert.match(String(by.get('POST-PRODUCT-OVERVIEW')?.copy||''),/七項/,'產品總覽必須說明七項文字產品知識');
assert.match(String(by.get('POST-PRODUCT-OVERVIEW')?.copy||''),/柒玄茶・龜鹿調飲粉/,'產品總覽缺第七項柒玄茶');
assert.doesNotMatch(String(by.get('POST-PRODUCT-OVERVIEW')?.copy||''),/六個正式產品/,'產品總覽不得回退舊六產品文案');
...
console.log(`PASS current post runtime: ${posts.length} posts, seven-product current copy, six approved media products, pending image work allowed only as pending_review, no stale product/image authority gates.`);
