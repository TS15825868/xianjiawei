...
def main():
    ...
    s=queue['summary']; assert s['generationRequiredActive']==465 and s['existingCandidateNeeds16PointReview']==21 and s['preflightRejectedNeedsRegeneration']==0 and s['preflightRejectedReplaced']==13 and s['publishedFinalLocked']==3 and s['campaignHold']==11 and 465+21+3+11==500
    c=manifest['counts']; assert c['runtimeTotal']==500 and c['activeImageGenerationRequired']==465 and c['existingCandidateNeeds16PointReview']==21 and c['preflightRejectedNeedsRegeneration']==0 and c['preflightRejectedReplaced']==13
    p=deploy['contentAuthority']; assert p['activeImageGenerationRequired']==465 and p['existingCandidateNeeds16PointReview']==21 and p['preflightRejectedNeedsRegeneration']==0 and p['preflightRejectedReplaced']==13
    assert deploy['candidatePreflight']['oldRejectedCount']==13 and deploy['candidatePreflight']['replacementGeneratedCount']==13 and set(deploy['candidatePreflight']['postIds'])==REPLACED
    ...
    for post_id,(asset_id,path) in REPLACEMENTS.items():
        assert batch['assets'][post_id]==path
    ...
    print('PASS preflight replacements: 13 old visuals quarantined, 13 dedicated replacement asset IDs registered; 465 generation / 21 review / 3 locked / 11 hold = 500.')
