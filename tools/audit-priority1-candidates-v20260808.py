...
def main():
 ...
 assert s['generationRequiredActive']==465 and s['existingCandidateNeeds16PointReview']==21
 assert s['preflightRejectedNeedsRegeneration']==0 and s['preflightRejectedReplaced']==13 and s['priority1GeneratedPendingReview']==6
 batch=next((x for x in data['priorityBatches'] if x.get('priority')==1),None); assert batch and batch['status']=='generated_pending_16_point_review' and batch['count']==6 and batch['assets']==EXPECTED
 ...
 print('PASS priority1 candidates: 6 exact-original SVG candidates remain intact; total queue 465 generation / 21 review / 13 preflight replacements')
