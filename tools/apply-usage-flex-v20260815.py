...
def normalize_usage(product:dict,pid:str):
 ...
 if pid=='guilu-drink-30':
  ...
  rest=keep(DRINK180,DRINK_TIME,OLD_DAY,'飲用份量與時間可依個人使用習慣與作息安排')
...
def update_official(rel:str,public=False):
 ...
 data['version']='2026-08-15-personal-routine-usage-v2'
 products={p.get('id'):p for p in data.get('products',[])}
 gao=products.get('guilu-gao') or {}
 ...
 if public and any('usagePrimary' in p for p in data.get('products',[])):
  d30['usagePrimary']=f'{DRINK30}；{DRINK_TIME}'; d180['usagePrimary']=f'{DRINK180}；{DRINK_TIME}'
 else:
  d30['usage_primary']=f'{DRINK30}；{DRINK_TIME}'; d180['usage_primary']=f'{DRINK180}；{DRINK_TIME}'
 rules=[str(x) for x in data.get('forbidden_rules',[]) if OLD_GAO not in str(x) and OLD_DAY not in str(x) and '一天一次' not in str(x) and '早晚各一小匙' not in str(x) and '每日1～2罐' not in str(x)]
 rules.extend([
  f'龜鹿膏不設定固定早上／下午時段；目前使用原則為「{GAO}」',
  f'龜鹿飲不設定固定白天時段；30cc目前使用方式為「{DRINK30}」，且{DRINK_TIME}',
  f'龜鹿飲180cc保留目前份量「{DRINK180}」，飲用時間依個人使用習慣與作息安排',
...
for rel in ['data.json','catalog-public.json','geo-data.json','content/visual-production-spec-current.json','content/ai-brand-control-v20260807.json','content/public-post-library.json']:
 ...
 if rel=='content/visual-production-spec-current.json':
  ...
  if isinstance(products,dict):
   ...
   products.setdefault('guilu-drink-180',{})['usage_primary']=f'{DRINK180}；{DRINK_TIME}'
...
for path in [*ROOT.glob('*.html'),ROOT/'site.js',ROOT/'site-product-data-authority.js',ROOT/'publishing-center-data-current-authority-guard.js',ROOT/'llms.txt',ROOT/'llms-full.txt']:
 if path.exists():
  ...
  if path.name=='site-product-data-authority.js':
   new=re.sub(r"normalized\.usage=\['每日 1-2罐'[^;]*;", "normalized.usage=['每日 1-2罐','可直接飲用或隔水溫熱後飲用','飲用時間可依個人使用習慣與作息安排','避免冰飲','開罐後請儘速飲用完畢'];", new, count=1)
   marker="if(product.id==='guilu-drink-180')"
   start=new.find(marker)
   if start>=0:
    pos=new.find('normalized.usage=',start)
    end=new.find(';',pos)
    if pos>=0 and end>=0:
     new=new[:pos]+"normalized.usage=['每日一包','可直接飲用或隔水溫熱後飲用','飲用時間可依個人使用習慣與作息安排','避免冰飲','開封後請儘速飲用完畢']"+new[end:]
   marker="if(product.id==='guilu-gao')"
   start=new.find(marker)
   if start>=0:
    pos=new.find('normalized.usage=',start)
    end=new.find(';',pos)
    if pos>=0 and end>=0:
     new=new[:pos]+"normalized.usage=['食用時間與份量可依個人使用習慣與作息安排','初次可先從半匙開始','可直接取用或加入約100～300mL溫熱水化開']"+new[end:]
...
t=t.replace('.trial-product-card__actions{display:grid;gap:12px;padding:8px 20px 20px;margin-top:auto}',
            '.trial-product-card__actions{display:grid;gap:12px;padding:10px 20px 20px;margin-top:auto}')
...
s=s.replace('每日1～2罐',DRINK30).replace('每日 1～2罐',DRINK30)
...
for rel in validator_files:
 ...
 v=v.replace(OLD_GAO,GAO).replace('每日一罐',DRINK30).replace('每日 1 罐',DRINK30).replace('每日1罐',DRINK30).replace('每日1～2罐',DRINK30).replace('每日 1～2罐',DRINK30).replace(OLD_DAY,DRINK_TIME)
...
assert DRINK_TIME in by['guilu-drink-180']['usage']
authority=(ROOT/'site-product-data-authority.js').read_text(encoding='utf-8')
assert "normalized.usage=['每日 1-2罐'" in authority
assert "normalized.usage=['每日一包','可直接飲用或隔水溫熱後飲用','飲用時間可依個人使用習慣與作息安排'" in authority
assert "normalized.usage=['食用時間與份量可依個人使用習慣與作息安排','初次可先從半匙開始','可直接取用或加入約100～300mL溫熱水化開'];" in authority
