...
- Rebuild broken legacy PNG aliases from the already-approved WebP assets.
...
PNG_MAP = {
    "guilu-gao-100g.png": DM / "guilu-gao-100g.webp",
...
def save_trial_jpeg() -> dict:
    if not TRIAL_SOURCE.is_file():
        raise SystemExit(f"缺少正式試喝母圖：{TRIAL_SOURCE}")
    with Image.open(TRIAL_SOURCE) as original:
        original.load()
