...
def main():
    ...
    require(counts.get("activeImageGenerationRequired") == 478, "待生成圖片數不是478")
    require(counts.get("existingCandidateNeeds16PointReview") == 8, "候選待16項審核數不是8")
    require(counts.get("preflightRejectedNeedsRegeneration") == 13, "預檢退回數不是13")

    queue_summary = queue.get("summary", {})
    require(queue.get("runtimeContentTotal") == 500, "圖片佇列runtimeContentTotal不是500")
    require(queue_summary.get("generationRequiredActive") == 478, "圖片佇列待生成數與manifest不一致")
    require(queue_summary.get("existingCandidateNeeds16PointReview") == 8, "圖片佇列候選數與manifest不一致")
    require(queue_summary.get("preflightRejectedNeedsRegeneration") == 13, "圖片佇列預檢退回數不是13")
    require(queue_summary.get("publishedFinalLocked") == 3, "已發布鎖定數不是3")
    require(queue_summary.get("campaignHold") == 11, "活動冷卻數不是11")
    require(478 + 8 + 3 + 11 == 500, "圖片狀態合計不是500")
    ...
    print("PASS post bank generators: base23 + v6 300 + v7 177 = 500; six current products only; image queue 478/8/3/11 with 13 preflight rejects")
