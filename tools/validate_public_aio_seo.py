...
def main() -> int:
    ...
    for filename in required_files:
        if not (ROOT / filename).exists():
            fail(errors, f"缺少機器可讀檔案：{filename}")
    ...
    for marker in ["100g", "30cc", "180cc", "75g", "600g", "鹿茸粉"]:
    ...
    for marker in ["仙加味", "萬華", "龜鹿膏", "龜鹿飲", "龜鹿湯塊", "龜鹿膠", "鹿茸粉"]:
        if marker not in geo_text:
            fail(errors, f"geo-data.json 缺少實體：{marker}")
    ...
    print(f"PASS AIO／SEO／GEO 正式合約：{len(urls)} 個 sitemap 頁面、Canonical、摘要、Schema 與機器可讀資料均通過")
    return 0
