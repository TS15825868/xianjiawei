...
OFFICIAL_SPECS = [
    ...
    f"龜鹿湯塊 {SOUP_MAIN_SPEC}",
    f"龜鹿膠 {JIAO_MAIN_SPEC}",
    "鹿茸粉 75g／罐",
]
...
# 只要求 canonical 公開頁進 sitemap；brand-origin.html canonical 指 brand.html，因此不重複列入。
REQUIRED_SITEMAP_PAGES = PRIMARY_DECISION_PAGES | {
    "dm.html", "recipes.html", "video.html", "knowledge.html", "hanfang-baike.html",
    "sources.html", "ingredients.html", "quality.html", "craft.html", "contact.html",
}
...
class HeadParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        if tag == "html": self.html_lang = data.get("lang", "")
            ...
            self._jsonld_open = True; self._jsonld_buffer = []
        elif tag == "h1": self.h1_count += 1

    def handle_endtag(self, tag: str) -> None:
        if tag == "title": self._in_title = False
        elif tag == "script" and self._jsonld_open:
            self.jsonld.append("".join(self._jsonld_buffer).strip()); self._jsonld_open = False; self._jsonld_buffer = []
...
def main() -> int:
    ...
    if missing_sitemap_pages: fail(errors, f"sitemap.xml 缺少主要canonical頁面：{missing_sitemap_pages}")
    ...
    for url in urls:
        ...
        source = path.read_text("utf-8"); parser = HeadParser(); parser.feed(source)
        description = meta(parser, name="description"); ai_summary = meta(parser, name="ai-summary"); robots = meta(parser, name="robots"); expected_canonical = f"{BASE}{filename}"
        ...
        if canonical(parser) != expected_canonical: fail(errors, f"{filename} canonical 錯誤：{canonical(parser)}")
        ...
        if "30cc玻璃瓶" in source or "小玻璃瓶" in source: fail(errors, f"{filename} 仍含30cc舊稱")
        bad_weights = unauthorized_soup_weights(source)
        if bad_weights: fail(errors, f"{filename} 出現未核准龜鹿湯塊重量：{bad_weights}")
    ...
    required_files=["robots.txt","llms.txt","llms-full.txt","catalog-public.json","geo-data.json","site-official-product-variants.js","content/public-post-library.json","content/public-asset-library.json","content/public-content-policy.json"]
    ...
    if len(catalog.get("products",[]))!=6: fail(errors,"catalog-public.json產品必須剛好六項")
    if catalog.get("officialSpecifications")!=OFFICIAL_SPECS: fail(errors,"catalog-public.json六項目前正式主規格不同步")
