...
def read(path:str):return (ROOT/path).read_text(encoding='utf-8')
def req(ok:bool,message:str)->None:
    if not ok:raise AssertionError(message)

def validate_posts()->None:
    doc=load('content/public-post-library.json');posts=doc.get('posts') or []
    ...
    for post in posts:
        status=str(post.get('status') or '');body=json.dumps(post,ensure_ascii=False)
        req('每日一罐' not in body or '不得回退成每日一罐' in body,f"貼文 {post.get('id')} 疑似回退30cc每日一罐")
        if '龜鹿膏' in body:req('早上＋下午' not in body and '每日早上及下午各一小匙' not in body and '早晚各一小匙' not in body,f"貼文 {post.get('id')} 龜鹿膏回退固定時段")
        if status=='published':
        ...
        if post.get('image_status') in {'needs_generation','replace-required'}:req(post.get('regeneration_mode')=='chatgpt_handoff',f"需換圖貼文未走ChatGPT回填：{post.get('id')}")
...
def validate_visual_review_policy()->None:
    ...
    deferred=visual.get('deferred_products') or {};req('qixuan-guilu-drink-powder' in deferred,'視覺權威必須保留柒玄茶通路分流註記')

def validate_no_retired_writebacks()->None:
    retired=[
        '.github/workflows/install-trial-poster-v9.yml','.github/workflows/install-trial-poster-v10.yml',
        '.github/workflows/install-trial-poster-v11.yml','.github/workflows/install-trial-poster-v12.yml',
        '.github/workflows/install-trial-poster-v14.yml','.github/workflows/audit-square-trial-assets-v12.yml',
        '.github/workflows/bump-site-entry-cache-v20260812.yml','tools/bump_site_entry_cache_v20260812.py',
