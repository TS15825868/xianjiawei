...
def normalize(value): return re.sub(r'^https?://[^/]+/xianjiawei/','',str(value or ''),flags=re.I).lstrip('/').split('?',1)[0].split('#',1)[0]
...
def current_authorities():
    ...
    if trial.get('status') in TRIAL_APPROVED_STATES and (trial.get('image') or trial.get('path')):approved_formal.add(normalize(trial.get('image') or trial.get('path')))
...
def validate_core_library(approved_products,approved_dms):
    ...
    for p in posts:
        ...
        if protected:
            ...
            if p.get('status')=='published':req(p.get('prevent_republish') is True and p.get('do_not_republish') is True,f'{pid} 已發布內容未完整鎖定')
            continue
        ...
        elif status=='needs_binary_sync':req(not p.get('publish_allowed'),f'{pid} binary待同步不得發布')
        elif status in {'candidate-review-required','live-check-required','approved-existing-pending-copy-review','approved_existing'}:req(bool(image),f'{pid} 有候選／正式參考狀態卻沒有圖片')
        ...
        if image:
            ...
            if any(ref in PRODUCTS for ref in refs) and len(refs)==1 and '/images/customer-display-v20260812/' in image:req(normalized in approved_products,f'{pid} 產品主圖不在目前formal authority核准產品圖清單：{normalized}')

        joined=f'{copy} {prompt} {image}'
        for marker in DEFERRED:req(marker not in joined,f'{pid} 公開候選重新出現暫緩產品：{marker}')
        ...
        if '龜鹿膏' in joined:req(not re.search(r'早上[＋+]下午|早晚各一小匙|每日早上及下午各一小匙',joined),f'{pid} 龜鹿膏又出現退役固定時段')
