(()=>{
  ...
  function productIds(p){
    ...
    if(refs.length)return[...new Set(refs)].slice(0,2);
    const t=`${p.title||''} ${p.copy||''}`;
    return Object.entries(PRODUCTS).filter(([,v])=>t.includes(v.name)||t.includes(v.name.replace('30cc玻璃罐','30cc'))||t.includes(v.name.replace('180cc鋁袋','180cc'))).map(([id])=>id).slice(0,2);
  }
  function eligible(p){
  ...
  function buildSvg(p){
    const ids=productIds(p),lines=titleLines(p.title||'仙加味日常'),seed=hash(p.id),a=150+seed%540,b=420+(seed>>>4)%280;
    ...
    let art='';
    if(ids.length){
      const w=ids.length===1?980:470;
      art=ids.map((id,i)=>{const v=PRODUCTS[id],x=ids.length===1?137:137+i*505;return `<rect x="${x}" y="402" width="${w}" height="536" rx="34" fill="#FFFDF9" stroke="#DED7CA" stroke-width="2"/><image href="${PUBLIC+v.img}" x="${x+32}" y="430" width="${w-64}" height="394" preserveAspectRatio="xMidYMid meet"/><text x="${x+w/2}" y="866" class="n" font-size="25" font-weight="700" text-anchor="middle">${esc(v.name)}</text><text x="${x+w/2}" y="908" class="g" font-size="21" text-anchor="middle">${esc(v.spec)}</text>`}).join('');
    }else{
