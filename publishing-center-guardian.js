(()=>{
  ...
  function scan(card){
    ...
    if(/needs|待生成|需重生成/i.test(imageState))errors.push('圖片：此篇已列入重新生成，換上新圖前不可審核通過');
  ...
  function enhance(){
    document.querySelectorAll('.post-card').forEach(card=>{
      ...
      if(result.errors.length){box.className='review-note warning';box.textContent=`品牌守門員阻擋：${result.errors.join('；')}`;if(approve){approve.disabled=true;approve.title='先完成重生成或修正守門員錯誤'}}
      else{box.className='review-note';box.textContent=`品牌守門員：文字與待生成狀態預檢通過。${result.notes.length?' '+result.notes.join(' '):' 圖片仍需完成16項人工檢查。'}`}
    })
  }
