...
/* 產品圖片安全層 2026-08-10：六項產品統一使用目前最新核准圖片；舊圖與30cc瓶裝錯版不得回到顧客頁。 */
(function(){
  ...
  function match(value){const text=String(value||'');if(/products-v3\//i.test(text))return'';for(const rule of RULES){if(rule.tests.some(test=>test.test(text)))return rule.key;}return'';}
  function officialFor(value){const key=match(value);return key?OFFICIAL[key]:'';}
  function forceImage(node,key){if(!node||!key||!OFFICIAL[key])return;node.setAttribute('src',OFFICIAL[key]);node.alt=ALT[key];node.style.objectFit='contain';node.style.objectPosition='center';node.style.width='100%';node.style.height='100%';node.style.maxWidth='100%';node.style.maxHeight='100%';node.style.transform='none';node.style.clipPath='none';node.dataset.xjwOfficialProduct=key;node.dataset.xjwProductPhoto='products-v3';node.dataset.xjwScalePolicy='uniform-only';}
  function repairNode(node,attr){const value=node.getAttribute(attr)||'';const key=match(value);if(!key)return;node.setAttribute(attr,OFFICIAL[key]);if(node.tagName==='IMG')forceImage(node,key);if(node.tagName==='A'){const label=(node.textContent||'').trim();if(/DM|海報|大圖|產品圖/.test(label))node.textContent='查看產品照片';node.title='查看產品照片';}}
  ...
  function forceKnownSurfaces(root=document){root.querySelectorAll?.('.product-card[data-product-id]').forEach(card=>{const key=keyFromProductCard(card),img=card.querySelector('.product-card__img img, img');if(key&&img)forceImage(img,key);});const detailKey=keyFromLocation();if(detailKey){document.querySelectorAll('.product-detail-hero__media img').forEach(img=>forceImage(img,detailKey));document.querySelectorAll('.product-detail-hero__media a').forEach(a=>{a.href=OFFICIAL[detailKey];a.title='查看產品照片';});}const modal=document.getElementById('product-modal');if(modal&&modal.classList.contains('show')){const key=keyFromModal(modal);if(key){modal.querySelectorAll('img').forEach(img=>forceImage(img,key));modal.querySelectorAll('a[href*="dm-final"],a[href*="products-v2"]').forEach(a=>{a.href=OFFICIAL[key];});}}}
  function repair(root){const scope=root?.querySelectorAll?root:document;scope.querySelectorAll('img[src], source[srcset]').forEach(node=>repairNode(node,node.tagName==='SOURCE'?'srcset':'src'));scope.querySelectorAll('a[href], [data-dm-src]').forEach(node=>{repairNode(node,'href');const dm=node.getAttribute('data-dm-src')||'',key=match(dm);if(key)node.setAttribute('data-dm-src',OFFICIAL[key]);});forceKnownSurfaces(scope);}
