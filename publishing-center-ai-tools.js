(()=>{
  ...
  function imagePrompt(p){return `請重新生成一張仙加味社群貼文主圖。\n\n貼文ID：${p.id||''}\n貼文標題：${p.title||''}\n貼文文案：${p.copy||''}\n分類：${p.category||''}\n季節：${p.season||'依文案判斷'}\n天氣：${p.weather||'依文案判斷'}\n場合：${p.occasion||'依文案判斷'}\n地點：${p.location||'依文案判斷'}\n圖片需求：${p.image_prompt||'依文案建立完全匹配的生活情境'}\n\n${BASE_RULES}\n\n圖片要求：只輸出一張獨立1:1社群主圖，不要網站截圖、ERP畫面或儀表板。繁體中文短標題即可，不要塞長文。產品如需出現，必須把正式原產品照片等比例合成，不得由AI重畫產品本體。小老闆如需出現，使用網站approved-v405同款Q版，姿勢依情境自由，不要為了湊角色而硬塞小鹿或小烏龜。生成後只作候選，回到待審核並完成16項檢查後才可核准。`}
  function fullPrompt(p){return `請把這篇仙加味貼文「文案＋圖片」整套重新生成，舊圖與舊文案都不要沿用。\n\n原貼文ID：${p.id||''}\n原標題：${p.title||''}\n原文案：${p.copy||''}\n\n${BASE_RULES}\n\n圖片要求：1:1社群主圖、繁體中文短標題；產品只能使用正式原產品照片等比例合成，AI只生成背景、角色、道具與情境，不得重畫產品包裝；小老闆姿勢自由但畫風固定；完成後先進待審核，不直接發布。\n\n請輸出：新標題、新正文、16項圖文一致性自檢結果、完整圖片生成指令。`}
  function enhance(){
