...
function status(p){if(locked(p))return'published';const r=rec(p.id);if(r.publishedAt)return'published';if(r.approvedAt)return'approved';return'pending'}
...
function promptFor(p){return `請只生成一張獨立1:1繁體中文社群貼文圖，不要網站畫面、不要儀表板。\n\n品牌只顯示：仙加味。不可出現「台興山產有限公司」、統編、公司電話或公司地址。\n貼文標題：${p.title}\n貼文文案：${p.copy}\n\n固定角色：只使用官網 images/brand/approved-v405/ 同款柔和立體Q版小老闆；圓臉、大而圓的深棕色眼睛、短黑髮、米白中式上衣、深綠圍裙、胸前紅色直式「仙加味」印章。姿勢可依情境自由變化，但臉型、髮型、服裝與整體畫風不可改。小鹿與小烏龜不是每張強制出現；情境需要夥伴時才加入，且必須是分開的獨立角色、同一Q版質感。灰色小河馬娃娃與米色小鹿安撫巾只在居家、休息、陪伴、親子等合適情境使用。\n視覺：暖米色、深藍、深綠、印章紅、少量金色；繁體中文；不談療效。\n產品：正式產品只有六項、六個規格。只能使用仙加味正式產品原圖等比例合成；AI只生成背景、角色、道具與情境。不可重畫、不可裁切、不可改包裝、文字、顏色、容量、規格或拉伸產品。\n固定產品規格與比例：\n1. 龜鹿膏：100g／罐，六角玻璃罐約51×78mm；只使用目前新版米白標籤。\n2. 龜鹿飲30cc玻璃罐：30cc／罐（小玻璃罐），裸罐、無貼紙、無外盒、無外袋、金色蓋；同型外觀約42mm直徑×51mm高，不得做高、做胖或稱瓶。\n3. 龜鹿飲180cc鋁袋：180cc／包（鋁袋），狹長直立，寬高比約0.64；同框時自然縮小，不得橫向拉寬或加高放大。\n4. 龜鹿湯塊：只有75g／盒深藍正式盒裝，8塊裝、每塊約9.375g；不得建立其他容量。\n5. 龜鹿膠：600g（1斤）／盒｜32塊裝｜每塊約18.75g；淡紫色正式盒裝依原圖比例，不可橫向拉長。\n6. 鹿茸粉：75g／罐，白色塑膠罐正式原圖。\n圖片需求：${p.image_prompt||'依文案建立完全匹配的生活情境。'}\n必須逐項匹配：品牌、產品、規格、價格／活動、季節、天氣、場合、地點、情境、環境、冷熱、表情、動作、小老闆與夥伴、比例尺寸、重複圖。\n不可出現與文案無關的產品、季節、天氣、地點、冰塊、冰飲、表情或動作。新圖只進待審核，不直接發布。`}
...
function expectation(p,k){
 ...
 const map={
  ...
  characters:'小老闆只使用官網approved-v405同款柔和立體Q版；姿勢可依情境自由變化，臉型、髮型、米白中式上衣、深綠圍裙與紅色直式仙加味印章不可改。小鹿與小烏龜不是每張強制出現；需要夥伴時才加入且必須分開。',
...
function openDialog(p,showUpload=false){if(!p)return;const r=rec(p.id),img=localImage(p);$('dialogContent').innerHTML=`<div class="dialog-grid"><div>${img?`<img class="dialog-image" src="${esc(img)}" alt="${esc(p.title)}">`:'<div class="state-card">尚無圖片</div>'}<p class="meta-line">貼文ID：${esc(p.id)}</p><p class="meta-line">圖片狀態：${state.images[p.id]?'本機ChatGPT新圖':esc(p.image_status||'未標示')}</p></div><div><h2>${esc(p.title)}</h2><pre class="copy-box">${esc(p.copy)}</pre>${!locked(p)?`<section class="visual-match-box"><h3>發布前十六項完整檢查</h3><p class="visual-match-intro">每一項都要實際查看圖片及文案後再勾選；產品比例需與原圖一致。</p><div class="visual-match-list">${DIMENSIONS.map(([k,l])=>`<label class="visual-match-item"><input type="checkbox" data-check="${k}" data-id="${esc(p.id)}" ${r.checks?.[k]?'checked':''}><strong>${l}</strong><span>${expectation(p,k)}</span></label>`).join('')}</div></section><div class="dialog-actions"><button class="button secondary" data-copy="${esc(p.id)}">複製文案</button><button class="button orange" data-chatgpt="${esc(p.id)}">圖不符合｜ChatGPT生成</button><label class="button green" for="replace-${esc(p.id)}">選擇新圖並換上</label><input id="replace-${esc(p.id)}" type="file" accept="image/*" data-replace="${esc(p.id)}" hidden><button class="button primary" data-approve="${esc(p.id)}">${r.approvedAt?'取消公開預審':'公開審核通過'}</button>${r.approvedAt?`<button class="button primary" data-now="${esc(p.id)}">匯入ERP草稿並發布</button>`:''}</div>${showUpload?'<p class="review-note warning">ChatGPT已開啟。生成完成後回到此頁，選擇新圖即可換上；新圖仍要完成十六項檢查。</p>':''}`:'<p class="review-note">這篇已發布鎖定，不得重新發布。</p>'}</div></div>`;$('postDialog').showModal()}
