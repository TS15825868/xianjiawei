...
(function(){
  ...
  function normalizeData(data){
    ...
    data.products=data.products.map(product=>{
      ...
      if(product.id==='guilu-drink-30'){
        ...
        normalized.usage=['每日一罐','可直接飲用或隔水溫熱後飲用','建議白天飲用','避免冰飲','開罐後請儘速飲用完畢'];
      }
      if(product.id==='guilu-drink-180'){
        normalized.size='180cc／包（鋁袋）';
        normalized.usage=['每日一包','可直接飲用或隔水溫熱後飲用','建議白天飲用','避免冰飲','開封後請儘速飲用完畢'];
      }
      if(product.id==='guilu-gao'){
        normalized.size='100g／罐';
        normalized.usage=['每日早上及下午各一小匙','初次可先從半匙開始','可直接取用或加入約100～300mL溫熱水化開','避免接近睡前食用'];
  ...
  const COPY_REPLACEMENTS=Object.freeze([
    ...
    ['建議早上與下午各一小匙','每日早上及下午各一小匙'],
    ['一般建議早上與下午各一小匙','每日早上及下午各一小匙']
  ]);
