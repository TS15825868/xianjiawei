...
(function(){
  ...
  const VERSION="20260809-public-clean-v2-knowledge-customer-copy";
  ...
  function removeTechnicalCards(){
    document.querySelectorAll('article.card,.card').forEach(card=>{
      ...
      const technicalTitle=/圖片規則|版本說明|更新說明|素材規則|開發說明|部署說明/.test(title);
      const technicalBody=/products-v\d|GitHub Web|AI重畫|核准原圖|實際尺寸鎖|不可拉寬|不可拉高|不得自行猜測尺寸|runtime|快取版本/.test(text);
  ...
  function removeImplementationParagraphs(){
    document.querySelectorAll('p,small').forEach(node=>{
      ...
      if(/^資料更新[:：]/.test(text)||/^內容版本[:：]?/.test(text)||/GitHub Web版已上線/.test(text)||/products-v\d.*(核准|正式原圖|比例)/.test(text))node.remove();
    });
  }
