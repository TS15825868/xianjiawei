...
(() => {
  ...
  function suppressDuplicateHomeMascot(page) {
    if (page !== HOME_PAGE) return false;
    const existing = document.getElementById("mascot-guide");
    if (existing) existing.remove();
    if (!document.getElementById("mascot-guide-suppressed")) {
      const marker = document.createElement("span");
      marker.id = "mascot-guide";
      marker.dataset.suppressed = "home-static-scene";
      marker.hidden = true;
      marker.setAttribute("aria-hidden", "true");
      const hero = document.querySelector("main .hero");
      (hero || document.body).insertAdjacentElement("afterend", marker);
    }
    return true;
  }
  ...
  function ensureSection(config) {
    ...
    if (section?.dataset?.suppressed) return null;
  ...
  function applyScene() {
    const page = document.body?.dataset?.page || HOME_PAGE;
    if (suppressDuplicateHomeMascot(page)) return true;

    const config = SCENES[page];
    if (!config) {
      document.getElementById("mascot-guide")?.remove();
      return true;
    }
    ...
    if (copy) {
      copy.querySelector(".eyebrow").textContent = config.eyebrow;
      copy.querySelector("h2").textContent = config.title;
      copy.querySelector("p:not(.eyebrow)").textContent = config.text;
    }
